...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer,
    ...
    """
    label_map = {label: i for i, label in enumerate(label_list)}  # 编码采用B-1与I-1的编码数值不一样
    features = []
    for (ex_index, example) in enumerate(examples):
        ...
        # tokens = tokenizer.tokenize(example.text_a)    # example: gui_id, text_a, label
        tokens = example.text_a if isinstance(example.text_a, list) else example.text_a.strip().split()
        label_ids = [label_map[x] for x in example.labels]  # TODO 没有标识是否为预测集，存在空字符label
        ...
        tokens += [sep_token]  # 对特征字符[SEP]添加当个字符的编码
        label_ids += [label_map['O']]
        segment_ids = [sequence_a_segment_id] * len(tokens)

        if cls_token_at_end:  # 用于区分特殊字符的拼接方式, [CLS] + A + [SEP]
            tokens += [cls_token]
            label_ids += [label_map['O']]
        ...
        input_ids = tokenizer.convert_tokens_to_ids(tokens)  # 进行编码序列化
        ...
        if pad_on_left:  # Padding Mask处理：segment_ids，表示第一个句子用0，1表示第二个句子
...
class JDNERProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_text(os.path.join(data_dir, "train.txt"), 'train'), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_text(os.path.join(data_dir, "dev.txt"), 'dev'), "dev")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            for x in line['labels']:
                if 'M-' in x:  # 防止标注错误：只采用B、I、O标注，而不是B、I、M、E、O
